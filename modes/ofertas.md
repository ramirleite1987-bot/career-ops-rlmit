...
For each offer: score each dimension, compute weighted total.
Final ranking + recommendation including time-to-offer considerations.

Ask the user for the offers if not already in context. Input can be text, URLs, or references to offers already evaluated in the tracker.
